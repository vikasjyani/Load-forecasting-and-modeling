...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Application State Management - Define first to avoid initialization errors
    const AppState = {
    ...
    if (analysisTypeSelect) {
        ...
        handleAnalysisTypeChange(); // Initialize
    }
    ...
    function renderAnalysisChart(analysisData) {
        ...
        const { chart_type, title, data, heatmap_data, matrix_data, axis_labels, unit } = analysisData;
        ...
        if (chartInfo && analysisData.metadata) {
            ...
            const fiscalYears = metadata.fiscal_years || [];
            const yearRange = fiscalYears.length > 1 ?
                `${Math.min(...fiscalYears)}-${Math.max(...fiscalYears)}` :
                fiscalYears[0] || 'Unknown';
        ...
        try {
            ...
            if (chart_type === 'heatmap') {
                renderHeatmapChart(analysisData);
            } else {
                let option = createEnhancedChartOption(analysisData);
    ...
    // Enhanced chart option creation
    function createEnhancedChartOption(analysisData) {
        const { chart_type, title, data, unit, peak_point, min_point, metadata } = analysisData;

        let option = {};

        switch (chart_type) {
            case 'line':
                option = createTimeSeriesLineOption(data, title, unit, peak_point, min_point);
                break;
            case 'line_dual_axis':
                option = createDualAxisLineOption(data, title, unit);
                break;
            case 'bar_line_combo':
            ...
            case 'line_with_markers':
                ...
                break;
            case 'bar':
                option = createBarChartOption(data, title, unit);
                break;
            default:
                console.warn('Unknown chart type:', chart_type);
                option = createTimeSeriesLineOption(data, title, unit);
        }

        return option;
    }

    // Enhanced time series line chart with peak/min markers
    function createTimeSeriesLineOption(data, title, unit, peak_point, min_point) {
        const baseOption = createLineChartOption(data, title, unit);

        // Add peak and minimum markers if available
        if (peak_point && min_point) {
            const seriesData = baseOption.series[0];

            // Add mark points for peak and minimum
            seriesData.markPoint = {
                data: [
                    {
                        name: 'Peak',
                        value: peak_point.demand,
                        xAxis: peak_point.datetime,
                        yAxis: peak_point.demand,
                        itemStyle: {
                            color: '#ff4757',
                            borderColor: '#fff',
                            borderWidth: 2
                        ...
                        label: {
                            show: true,
                            position: 'top',
                            formatter: `Peak: ${peak_point.demand.toFixed(1)} ${unit}`
                        }
                    },
                    {
                        name: 'Minimum',
                        value: min_point.demand,
                        xAxis: min_point.datetime,
                        yAxis: min_point.demand,
                        itemStyle: {
                            color: '#2ed573',
                            borderColor: '#fff',
                            borderWidth: 2
                        ...
                        label: {
                            show: true,
                            position: 'bottom',
                            formatter: `Min: ${min_point.demand.toFixed(1)} ${unit}`
                        }
                    }
                ]
            };
    ...
    // Dual axis chart for load factor analysis
    function createDualAxisLineOption(data, title, unit) {
        return {
            title: {
                text: title,
                left: 'center',
    ...
    function createBarLineComboOption(data, title, unit) {
        return {
            ...
            series: data.datasets?.map((dataset, index) => ({
                name: dataset.label,
                type: dataset.type || (dataset.label.includes('Load Factor') ? 'line' : 'bar'),
                yAxisIndex: dataset.yAxisID === 'y1' ? 1 : 0,
                data: dataset.data,
                itemStyle: {
    ...
    function renderHeatmapChart(analysisData) {
        const { heatmap_data, matrix_data, axis_labels, title, unit } = analysisData;

        // Use Plotly for heatmap if available
        if (typeof Plotly !== 'undefined' && heatmap_data) {
            const plotlyData = [{
                z: matrix_data,
                x: axis_labels?.x || [],
                y: axis_labels?.y || [],
            ...
            const layout = {
                ...
                xaxis: {
                    title: axis_labels?.x_title || 'X Axis',
                    tickangle: -45
                },
                yaxis: {
                    title: axis_labels?.y_title || 'Y Axis'
                },
                height: 500,
                margin: { t: 80, r: 80, b: 80, l: 80 }
            };

            Plotly.newPlot('mainChart', plotlyData, layout, {
                responsive: true,
                displayModeBar: true,
                modeBarButtonsToRemove: ['pan2d', 'lasso2d', 'select2d']
            });
        } else {
            // Fallback to ECharts heatmap (requires additional setup)
            console.warn('Plotly not available for heatmap. Consider using ECharts heatmap plugin.');
            showAlert('warning', 'Heatmap visualization requires Plotly.js library.');
        }
    }

    // Enhanced analysis type selection handler
    function handleAnalysisTypeChange() {
        ...
        if (analysisType && descriptionContainer && descriptionText) {
            // Get analysis type info from dashboard data
            const analysisInfo = getAnalysisTypeInfo(analysisType);

            if (analysisInfo) {
                descriptionText.innerHTML = `
                <strong>${analysisInfo.description}</strong><br>
                <small class="text-muted">
                    <i class="fas fa-chart-${getChartIcon(analysisInfo.chart_type)} me-1"></i>
                    Chart Type: ${analysisInfo.chart_type} | 
                    <i class="fas fa-database me-1"></i>
                    ${analysisInfo.data_requirements}
                </small>
            `;
                descriptionContainer.classList.remove('d-none');

                // Check data suitability
                if (AppState && AppState.currentProfile) {
                    checkAnalysisSuitability(AppState.currentProfile, analysisType);
                }
            } else {
                descriptionContainer.classList.add('d-none');
            }
    ...
    function getAnalysisTypeInfo(analysisType) {
        const analysisTypes = {
            'overview': {
                description: 'Complete time series with peak/minimum identification and key statistics',
                chart_type: 'line',
                data_requirements: 'Hourly data recommended'
            },
            'heatmap_hour_day': {
                description: 'Hour vs day-of-week heatmap showing weekly load patterns',
                chart_type: 'heatmap',
                data_requirements: 'At least 1 week of data'
            },
            'heatmap_month_year': {
                description: 'Month vs year heatmap revealing seasonal trends',
                chart_type: 'heatmap',
                data_requirements: 'At least 1 year of data'
            },
            'peak_analysis': {
                description: 'Identification and analysis of top peak demand events',
                chart_type: 'bar',
                data_requirements: 'Any duration'
            },
            'load_factor': {
                description: 'System efficiency analysis showing load factor evolution',
                chart_type: 'line',
                data_requirements: 'Multi-month data recommended'
            },
            'duration_curve': {
                description: 'Capacity utilization analysis with percentiles',
                chart_type: 'line',
                data_requirements: 'Representative period'
            },
            'monthly': {
                description: 'Month-wise load comparison with statistics',
                chart_type: 'bar',
                data_requirements: 'At least 3 months'
            },
            'daily_profiles': {
                description: 'Sample daily load curves showing patterns',
                chart_type: 'line',
    ...
    // Get chart icon for analysis type
    function getChartIcon(chartType) {
        ...
        return icons[chartType] || 'chart-area';
    }

    // Check analysis suitability for selected profile
    async function checkAnalysisSuitability(profileId, analysisType) {
        try {
            const response = await fetchWithRetry(
                `${API_CONFIG.base}/analysis_suitability/${profileId}/${analysisType}`
            );
            const result = await response.json();

            if (result.status === 'success') {
                const suitability = result.data;
                displaySuitabilityWarnings(suitability);
            }
        } catch (error) {
            console.warn('Could not check analysis suitability:', error);
        }
    }

    // Display suitability warnings
    function displaySuitabilityWarnings(suitability) {
        const warningContainer = document.getElementById('analysisWarnings') || createWarningContainer();

        if (!suitability.suitable) {
            warningContainer.innerHTML = `
            <div class="alert alert-warning">
                <i class="fas fa-exclamation-triangle me-2"></i>
                <strong>Not Suitable:</strong> ${suitability.reason}
            </div>
        `;
            warningContainer.classList.remove('d-none');
        } else if (suitability.warnings && suitability.warnings.length > 0) {
            warningContainer.innerHTML = `
            <div class="alert alert-info">
                <i class="fas fa-info-circle me-2"></i>
                <strong>Note:</strong> ${suitability.warnings.join('; ')}
            </div>
        `;
            warningContainer.classList.remove('d-none');
        } else {
        ...
        if (suitability.recommendations && suitability.recommendations.length > 0) {
            const recommendationContainer = document.getElementById('analysisRecommendations') ||
                createRecommendationContainer();
            recommendationContainer.innerHTML = `
            <div class="alert alert-success">
                <i class="fas fa-lightbulb me-2"></i>
                <strong>Recommendations:</strong> ${suitability.recommendations.join('; ')}
            </div>
        `;
    ...
    function createWarningContainer() {
        ...
        container.id = 'analysisWarnings';
        container.className = 'd-none';

        const analysisControls = document.getElementById('analysisControls');
        if (analysisControls) {
            analysisControls.appendChild(container);
        }

        return container;
    }

    // Enhanced data table update for different analysis types
    function updateDataTable(analysisData) {
        ...
        const { chart_type, summary, metadata } = analysisData;
        let tableData = [];
        let columns = [];

        try {
            // Handle different analysis types
            if (chart_type === 'bar_line_combo' && summary) {
                // Monthly analysis data
                tableData = Object.entries(summary).map(([month, stats]) => ({
                    month: month,
                    average: stats.mean?.toFixed(2) || '--',
                    maximum: stats.max?.toFixed(2) || '--',
                    minimum: stats.min?.toFixed(2) || '--',
                    load_factor: stats.load_factor?.toFixed(2) || '--',
                ...
                columns = [
                    ...
                    { title: `Average (${metadata.unit})`, data: 'average', className: 'text-end' },
                    { title: `Peak (${metadata.unit})`, data: 'maximum', className: 'text-end' },
                    { title: `Minimum (${metadata.unit})`, data: 'minimum', className: 'text-end' },
                    { title: 'Load Factor (%)', data: 'load_factor', className: 'text-end' },
                    { title: `Std Dev (${metadata.unit})`, data: 'std_dev', className: 'text-end' },
                    { title: 'Data Points', data: 'data_points', className: 'text-end' }
                ];
            } else if (chart_type === 'bar' && analysisData.peak_statistics) {
                // Peak analysis data
                const peakStats = analysisData.peak_statistics;
                tableData = [{
                    metric: 'Peak Events Analysis',
                    total_events: peakStats.total_peak_events || '--',
                    threshold: `${peakStats.peak_threshold?.toFixed(2) || '--'} ${metadata.unit}`,
                    average_peak: `${peakStats.average_peak?.toFixed(2) || '--'} ${metadata.unit}`,
                    highest_peak: `${peakStats.highest_peak?.toFixed(2) || '--'} ${metadata.unit}`
                }];

                columns = [
                    { title: 'Analysis', data: 'metric' },
                    { title: 'Total Events', data: 'total_events', className: 'text-end' },
                    { title: 'Threshold', data: 'threshold', className: 'text-end' },
                    { title: 'Average Peak', data: 'average_peak', className: 'text-end' },
                    { title: 'Highest Peak', data: 'highest_peak', className: 'text-end' }
                ];
            } else if (analysisData.percentiles) {
                // Duration curve percentiles
                tableData = Object.entries(analysisData.percentiles).map(([percentile, value]) => ({
                    percentile: percentile.toUpperCase(),
                    value: `${value.toFixed(2)} ${metadata.unit}`,
                    percentage: percentile.replace('p', '') + '%'
                }));

                columns = [
                    { title: 'Percentile', data: 'percentile' },
                    { title: 'Percentage', data: 'percentage', className: 'text-center' },
                    { title: `Demand (${metadata.unit})`, data: 'value', className: 'text-end' }
                ];
            } else {
                // Default: show basic metadata
                tableData = [{
                    metric: 'Analysis Summary',
                    type: metadata.analysis_type || '--',
                    data_points: metadata.data_points?.toLocaleString() || '--',
                    unit: metadata.unit || '--',
                ...
                columns = [
                    ...
                    { title: 'Analysis Type', data: 'type' },
                    { title: 'Data Points', data: 'data_points', className: 'text-end' },
                    { title: 'Unit', data: 'unit', className: 'text-center' },
                    { title: 'Generated', data: 'generated', className: 'text-nowrap' }
                ];
            }
