...
class DemandVisualizationApp {
    ...
    async loadExistingTdLosses() {
        try {
            ...
            if (result.success && result.config.td_losses && result.config.td_losses.length > 0) {
                this.state.tdLosses = result.config.td_losses;
            } else {
                // Default T&D losses if API fails, no data, or empty array from API
                this.state.tdLosses = [
                    { year: this.state.filters.startYear || 2025, loss_percentage: 12.0 },
                    { year: Math.floor(((this.state.filters.startYear || 2025) + (this.state.filters.endYear || 2037)) / 2) || 2030, loss_percentage: 8.0 },
            ...
            // Fallback to default T&D losses on any error
            this.state.tdLosses = [
                { year: this.state.filters.startYear || 2025, loss_percentage: 12.0 },
                { year: Math.floor(((this.state.filters.startYear || 2025) + (this.state.filters.endYear || 2037)) / 2) || 2030, loss_percentage: 8.0 },
                { year: this.state.filters.endYear || 2037, loss_percentage: 6.0 }
            ];
            this.updateTdLossesDisplay(); // Ensure display is updated with defaults
        }
    }
