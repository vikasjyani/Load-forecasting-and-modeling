document.addEventListener('DOMContentLoaded', function() {
    ...
    async function initializeColorManager() {
        try {
            state.colorManager = window.colorManager;
            if (state.colorManager) {
                await state.colorManager.waitForInitialization();
                
                // Register for color change events
                state.colorManager.registerColorChangeCallback((category, item, color) => {
                    handleColorChange(category, item, color);
                });
                
                console.log('✅ PyPSA ColorManager initialized successfully');
            }
        } catch (error) {
            console.error('❌ PyPSA ColorManager initialization failed:', error);
        }
    }

    // Handle color changes from centralized color manager
    function handleColorChange(category, item, color) {
        // Update local color palette for backward compatibility
        if (category === 'carriers') {
            state.colorPalette[item] = color;
        }
        
        // Refresh any visible charts that use this color
        refreshChartsWithNewColors();
    }

    // Get color from centralized manager with fallback
    function getColor(category, item, defaultColor = null) {
        if (state.colorManager && state.colorManager.initialized) {
            return state.colorManager.getColor(category, item, defaultColor || getRandomColor());
        }
        
        // Fallback to local palette or random color
        return state.colorPalette[item] || defaultColor || getRandomColor();
    }

    // Refresh charts when colors change
    function refreshChartsWithNewColors() {
        // This will be called when colors are updated from the settings page
        // We can add specific chart refresh logic here if needed
        console.log('🎨 Colors updated, charts may need refresh');
    }
    ...
    function createDispatchStackPlot() {
            ...
            carriers.forEach(carrier => {
                ...
                if (yValues.some(v => Math.abs(v) > 1e-6)) {
                    traces.push({
                        ...
                        fillcolor: getColor('carriers', carrier),
                ...
                const yValues = combinedStorageData.map(item => item[col] || 0);
                 if (yValues.some(v => v > 1e-6)) {
                    traces.push({
                        ...
                        fillcolor: getColor('carriers', col.replace(' Discharge', '')) || getColor('carriers', col),
                        line: { width: 0 }, fill: 'tonexty',
                ...
                const yValues = combinedStorageData.map(item => item[col] || 0);
                 if (yValues.some(v => v < -1e-6)) {
                    traces.push({
                        ...
                        fillcolor: getColor('carriers', col.replace(' Charge', '')) || getColor('carriers', col),
                        line: { width: 0 }, fill: 'tonexty',
                        hovertemplate: `%{x|%Y-%m-%d %H:%M}<br>${col}: %{y:,.1f} MW<extra></extra>`
                    });
                }
        ...
        if (load && load.length > 0) {
            ...
            traces.push({
                ...
                line: { color: getColor('carriers', 'Load', 'black'), width: 2.5 },
            ...
            if (yValues.some(v => Math.abs(v) > 1e-3)) {
                if (isLoad) {
                    ...
                    traces.push({
                        ...
                        fillcolor: getColor('carriers', compName.replace(' Charge','').replace(' Discharge','')) || getColor('carriers', compName),
                        line: { width: 0 }, fill: 'tonexty',
    ...
    function createCapacityByCarrierPlot() {
        ...
        const trace = {
            ...
            type: 'bar',
            marker: { color: capacityData.map(item => getColor('carriers', item.Carrier)) },
    ...
    function createNewCapacityAdditionsPlot() {
        ...
        const trace = {
            ...
            type: 'bar',
            marker: { color: additionsData.map(item => getColor('carriers', item.Carrier)) },
    ...
    function createCUFPlot() {
        ...
        const trace = {
            ...
            type: 'bar',
            marker: { color: cufData.map(item => getColor('carriers', item.Carrier)) },
    ...
    function createCurtailmentPlot() {
        ...
        const trace = {
            ...
            type: 'bar',
            marker: { color: curtailmentData.map(item => getColor('carriers', item.Carrier)) },
        ...
        storageTypes.forEach(type => {
            ...
            if (yValues.some(v => Math.abs(v) > 1e-3)) {
                ...
                traces.push({
                    ...
                    line: { color: getColor('carriers', baseColorKey) || getColor('carriers', type), width: 2 },
    ...
    function createStorageUtilizationPlot() {
        ...
        const trace1 = {
            ...
            marker: { color: getColor('carriers', 'Storage Charge') || getColor('carriers', 'Store Charge', 'rgba(255,165,0,0.8)') }
        };
        const trace2 = {
            x: storageStats.map(item => item.Storage_Type),
            y: storageStats.map(item => item.Discharge_MWh),
            name: 'Discharge (MWh)', type: 'bar',
            marker: { color: getColor('carriers', 'Storage Discharge') || getColor('carriers', 'Store Discharge', 'rgba(50,205,50,0.8)') }
        };
        const layout = {
    ...
    function createEmissionsByCarrierPlot() {
        ...
        const trace = {
            ...
            type: 'bar',
            marker: { color: emissionsData.map(item => getColor('carriers', item.Carrier)) },
    ...
    function renderComparisonPlot(comparisonResult, mainPlotContainer, secondaryPlotContainer, secondaryRow) {
        ...
        const effectiveColors = colors || state.colorPalette || {};
            ...
            // Update effective colors with centralized color manager
            if (state.colorManager && state.colorManager.initialized) {
                Object.keys(effectiveColors).forEach(key => {
                    effectiveColors[key] = getColor('carriers', key, effectiveColors[key]);
                });
            }
        ...
        if (type === 'capacity' || type === 'generation' || type === 'new_capacity_additions') {
            ...
            Array.from(uniqueCarriers).forEach(carrier => {
                ...
                traces.push({
                    ...
                    type: 'bar',
                    marker: {
                        color: getColor('carriers', carrier, effectiveColors[carrier])
                    }
                });
            });
            ...
            if (cufPlotData.length > 0) {
                ...
                Array.from(cufUniqueCarriers).forEach(carrier => {
                    ...
                    cufTraces.push({
                        ...
                        name: carrier,
                        type: 'bar',
                        marker: { color: getColor('carriers', carrier, effectiveColors[carrier]) }
                    });
                });
                const cufLayout = {
    ...
    // Initialize ColorManager when DOM is ready
