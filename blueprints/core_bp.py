...
class CoreBlueprint(ServiceBlueprint):
    ...
    def register_routes(self):
        ...
        @self.blueprint.route('/api/colors/analyze-project', methods=['POST'])
        @api_route()
        def analyze_project_colors_api():
            return self._analyze_project_colors()
        
        # Project Management APIs
        @self.blueprint.route('/project/create', methods=['POST'])
        @api_route(require_project_check=False)
        def create_project_api():
            return self._create_project()
        
        @self.blueprint.route('/project/load', methods=['POST'])
        @api_route(require_project_check=False)
        def load_project_api():
            return self._load_project()
        
        @self.blueprint.route('/project/validate', methods=['GET', 'POST'])
        @api_route(require_project_check=False)
    ...
    def _analyze_project_colors(self):
        """Analyze current project and assign colors to new items"""
        try:
            ...
            if not project_path:
                return error_json("No project is currently loaded")
            
            if not os.path.exists(project_path):
                return error_json("Current project path does not exist")
            
            # Initialize color manager
            color_manager = ColorManager()
            
            # Analyze project and assign colors
            result = color_manager.analyze_project_colors(project_path)
            ...
            if result['success']:
                ...
                return error_json(f"Project analysis failed: {result.get('error', 'Unknown error')}")
                
        except Exception as e:
            logger.exception(f"Error analyzing project colors: {e}")
            return error_json(f"Failed to analyze project: {str(e)}")
    ...
    def _create_project(self) -> Dict[str, Any]:
        ...
        try:
            ...
            project_location = request.form.get('projectLocation', '').strip()
            
            # Validate inputs
            if not project_name:
                return error_json("Project name is required", status_code=400)
            
            if not project_location:
                return error_json("Project location is required", status_code=400)
            ...
            if os.path.exists(project_path):
                return error_json(f"Project '{project_name}' already exists at this location", status_code=400)
            ...
            try:
                ...
                subdirs = [
                    'inputs',
                    'results',
                    'results/demand_projection',
                    'results/load_profiles', 
                    'results/PyPSA_Modeling',
                    'results/Pypsa_results',
                    'logs',
                    'config'
                ]
                ...
                config_data = {
                    ...
                    'version': '1.0.0',
                    'description': f'Energy forecasting project: {project_name}'
                }
                ...
                return success_json(
                    ...
                    data={
                        ...
                        'created_directories': subdirs
                    }
                )
                
            except OSError as e:
                logger.error(f"Error creating project directories: {e}")
                return error_json(f"Failed to create project directories: {str(e)}", status_code=500)
                
        except Exception as e:
            logger.exception(f"Error creating project: {e}")
            return error_json(f"Failed to create project: {str(e)}", status_code=500)
    ...
    def _load_project(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not project_path:
                return error_json("Project path is required", status_code=400)
            
            # Validate project exists
            if not os.path.exists(project_path):
                return error_json("Project path does not exist", status_code=400)
            
            # Validate project structure
            validation_result = self._validate_project_structure(project_path)
            if not validation_result['valid']:
                return error_json(
                    f"Invalid project structure: {validation_result['error']}",
                    status_code=400
                )
            ...
            return error_json(f"Failed to load project: {str(e)}", status_code=500)
    ...
    def _validate_project(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if request.method == 'GET':
                ...
                if request.is_json:
                    project_path = request.json.get('path', '').strip()
                else:
                    project_path = request.form.get('path', '').strip()
            ...
            if validation_result['valid']:
                ...
                return error_json(
                    f"Project structure validation failed: {validation_result['error']}",
                    data=validation_result,
                    status_code=400
                )
            ...
            return error_json(f"Failed to validate project: {str(e)}", status_code=500)
    
    def _validate_project_structure(self, project_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            required_dirs = ['inputs', 'results', 'logs', 'config']
            missing_dirs = []
            ...
            # Check for results subdirectories
            results_subdirs = ['demand_projection', 'load_profiles', 'PyPSA_Modeling', 'Pypsa_results']
            missing_result_dirs = []
            
            for subdir in results_subdirs:
                subdir_path = os.path.join(project_path, 'results', subdir)
                if not os.path.exists(subdir_path):
                    missing_result_dirs.append(subdir)
            
            # Create missing results subdirectories if needed
            if missing_result_dirs:
                try:
                    for subdir in missing_result_dirs:
                        os.makedirs(os.path.join(project_path, 'results', subdir), exist_ok=True)
                except OSError as e:
                    return {
                        'valid': False,
                        'error': f'Could not create missing result directories: {str(e)}'
                    }
            ...
            return {
                ...
                'created_missing_dirs': missing_result_dirs if missing_result_dirs else None
            }
