...
def create_project_route(): # Renamed to avoid conflict with import
    ...
    try:
        ...
        result = create_project_structure(project_path, current_app.config['TEMPLATE_FOLDER'])
        if result['success']:
            ...
            current_app.logger.error(f"Failed to create project structure at {project_path}: {result.get('message', 'Unknown error')}")
            return jsonify({
                'status': 'error',
                'message': result.get('message', 'Failed to create project structure. Check server logs for details.')
            })
    except Exception as e:
...
# Add error handlers for the project blueprint to ensure JSON responses
@project_bp.errorhandler(400)
def handle_bad_request(e):
    current_app.logger.warning(f"400 Bad Request in project_bp: {request.url} - {str(e)}")
    return jsonify({'status': 'error', 'message': 'Bad request'}), 400

@project_bp.errorhandler(404)
def handle_not_found(e):
    current_app.logger.warning(f"404 Not Found in project_bp: {request.url} - {str(e)}")
    return jsonify({'status': 'error', 'message': 'Resource not found'}), 404

@project_bp.errorhandler(500)
def handle_internal_error(e):
    current_app.logger.exception(f"500 Internal Server Error in project_bp: {request.url} - {str(e)}")
    return jsonify({'status': 'error', 'message': 'Internal server error'}), 500

@project_bp.errorhandler(Exception)
def handle_generic_exception(e):
    current_app.logger.exception(f"Unhandled exception in project_bp: {str(e)}")
