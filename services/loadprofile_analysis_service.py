...
from utils.service_cache_mixin import ServiceCacheMixin

logger = logging.getLogger(__name__)

class LoadProfileAnalysisService(ServiceCacheMixin):
    """
    ...
    def __init__(self, project_path: str):
        ...
        # Initialize cache using mixin
        self.__init_cache__(cache_ttl=300)
    ...
    def get_available_profiles(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Cache result
            self._set_cached(cache_key, enhanced_profiles)
    ...
    def _perform_overview_analysis(self, df: pd.DataFrame, unit: str, parameters: Dict) -> Dict:
        """Enhanced overview analysis with time series plotting"""
        from utils.plot_utils import initialize_load_profile_plots, plot_utils
        
        load_plots = initialize_load_profile_plots(plot_utils)
        
        # Create main time series chart
        chart_result = load_plots.time_series_line_plot(
            df, 
        ...
        # Find peak and minimum points for annotation
        # Apply unit conversion for display
        unit_factor = self.analyzer.unit_factors.get(unit, 1)
        ...
        peak_point = {
            'index': peak_idx,
            'demand': float(demand_values.loc[peak_idx]),
            'datetime': df.loc[peak_idx, 'ds'].strftime('%Y-%m-%d %H:%M') if 'ds' in df.columns else str(peak_idx)
        }
        
        min_point = {
            'index': min_idx,
            'demand': float(demand_values.loc[min_idx]),
            'datetime': df.loc[min_idx, 'ds'].strftime('%Y-%m-%d %H:%M') if 'ds' in df.columns else str(min_idx)
        }
        
        return {
            'chart_type': 'line',
            'title': f'Load Profile Overview ({unit})',
            'data': chart_result.get('chart_data', {}),
    ...
    def _perform_heatmap_analysis(self, df: pd.DataFrame, unit: str, parameters: Dict) -> Dict:
        ...
        # Default to hour-day heatmap
        heatmap_type = parameters.get('heatmap_type', 'hour_day')
        
        if heatmap_type == 'hour_day':
            chart_result = load_plots.hour_day_heat_map(df, title=f"Daily Load Patterns ({unit})")
        elif heatmap_type == 'month_year':
            chart_result = load_plots.month_year_heat_map(df, title=f"Seasonal Load Patterns ({unit})")
        elif heatmap_type == 'seasonal':
            chart_result = load_plots.seasonal_heat_map(df, title=f"Seasonal Hourly Patterns ({unit})")
        else:
            chart_result = load_plots.heat_map(df, title=f"Load Pattern Heatmap ({unit})", unit=unit)
        
        return {
            'chart_type': 'heatmap',
            'title': chart_result.get('chart_data', {}).get('options', {}).get('plugins', {}).get('title', {}).get('text', 'Load Heatmap'),
            'data': chart_result.get('chart_data', {}),
            'heatmap_data': chart_result.get('heatmap_data', []),
            'matrix_data': chart_result.get('matrix_data', []),
            'axis_labels': {
                'x': chart_result.get('x_labels', []),
                'y': chart_result.get('y_labels', [])
            },
            'unit': unit,
            'metadata': {
    ...
    def _perform_peak_analysis(self, df: pd.DataFrame, unit: str, parameters: Dict) -> Dict:
        """Enhanced peak analysis with visualization"""
        from utils.plot_utils import initialize_load_profile_plots, plot_utils
        ...
        chart_result = load_plots.peak_demand_analysis_chart(
            df,
            top_n=top_n,
            title=f"Top {top_n} Peak Demand Events ({unit})"
        )
        
        # Calculate peak statistics
        peak_stats = self._calculate_enhanced_peak_statistics(df, unit)
        
        return {
            'chart_type': 'bar',
            'title': f'Peak Demand Analysis ({unit})',
            'data': chart_result.get('chart_data', {}),
    ...
    def _perform_load_factor_analysis(self, df: pd.DataFrame, unit: str, parameters: Dict) -> Dict:
        ...
        chart_result = load_plots.load_factor_trend_chart(
            df,
            period=period,
            title=f"Load Factor Trend Analysis - {period.title()} ({unit})"
        )
        
        # Calculate load factor statistics
        lf_stats = self._calculate_load_factor_statistics(df, period)
        
        return {
            'chart_type': 'line_with_markers',
            'title': f'Load Factor Analysis - {period.title()}',
            'data': chart_result.get('chart_data', {}),
            'load_factor_stats': lf_stats,
            'unit': unit,
            'metadata': {
    ...
    def _perform_duration_curve_analysis(self, df: pd.DataFrame, unit: str, parameters: Dict) -> Dict:
        ...
        chart_result = load_plots.load_duration_curve(
            df,
            title=f"Load Duration Curve ({unit})"
        )
        
        # Calculate duration curve statistics
        # Apply unit conversion
        unit_factor = self.analyzer.unit_factors.get(unit, 1)
        demand_data = (df['demand'] * unit_factor).dropna()
        percentiles = {
            'p10': float(np.percentile(demand_data, 10)),
            'p25': float(np.percentile(demand_data, 25)),
            'p50': float(np.percentile(demand_data, 50)),
            'p75': float(np.percentile(demand_data, 75)),
            'p90': float(np.percentile(demand_data, 90)),
            'p95': float(np.percentile(demand_data, 95)),
            'p99': float(np.percentile(demand_data, 99))
        }
    ...
    def _perform_monthly_analysis(self, df: pd.DataFrame, unit: str, parameters: Dict) -> Dict:
        ...
        chart_result = load_plots.monthly_comparison_chart(
            df,
            title=f"Monthly Load Comparison ({unit})"
        )
        ...
        if 'ds' not in df_monthly.columns:
            raise ValueError("Missing 'ds' (datetime) column in DataFrame")
        if 'demand' not in df_monthly.columns:
            raise ValueError("Missing 'demand' column in DataFrame")
            
        df_monthly['ds'] = pd.to_datetime(df_monthly['ds'])
        df_monthly['Month'] = df_monthly['ds'].dt.month
        df_monthly['Month_Name'] = df_monthly['ds'].dt.month_name()
        
        # Apply unit conversion
        unit_factor = self.analyzer.unit_factors.get(unit, 1)
        df_monthly['demand_converted'] = df_monthly['demand'] * unit_factor
        
        monthly_stats = df_monthly.groupby(['Month', 'Month_Name'])['demand_converted'].agg([
            'mean', 'max', 'min', 'std', 'count'
        ]).reset_index()
        
        monthly_stats['load_factor'] = (monthly_stats['mean'] / monthly_stats['max'] * 100).round(2)
        monthly_stats['cv'] = (monthly_stats['std'] / monthly_stats['mean'] * 100).round(2)
        ...
        for _, row in monthly_stats.iterrows():
            ...
            summary[month_name] = {
                ...
                'coefficient_variation': float(row['cv']),
                'data_points': int(row['count'])
            }
        
        return {
            'chart_type': 'bar_line_combo',
            'title': f'Monthly Load Analysis ({unit})',
            'data': chart_result.get('chart_data', {}),
            'summary': summary,
            'unit': unit,
            'metadata': {
    ...
    def _perform_daily_profiles_analysis(self, df: pd.DataFrame, unit: str, parameters: Dict) -> Dict:
        """Daily load profiles analysis"""
        from utils.plot_utils import initialize_load_profile_plots, plot_utils
        
        load_plots = initialize_load_profile_plots(plot_utils)
        
        sample_days = parameters.get('sample_days', 7)
        
        chart_result = load_plots.daily_load_profile_chart(
            df,
            sample_days=sample_days,
            title=f"Sample Daily Load Profiles ({unit})"
        )
        ...
        if 'ds' not in df_daily.columns:
            raise ValueError("Missing 'ds' (datetime) column in DataFrame")
        if 'demand' not in df_daily.columns:
            raise ValueError("Missing 'demand' column in DataFrame")
            
        df_daily['ds'] = pd.to_datetime(df_daily['ds'])
        df_daily['Hour'] = df_daily['ds'].dt.hour
        
        # Apply unit conversion
        unit_factor = self.analyzer.unit_factors.get(unit, 1)
        df_daily['demand_converted'] = df_daily['demand'] * unit_factor
        
        hourly_avg = df_daily.groupby('Hour')['demand_converted'].agg([
            'mean', 'std', 'min', 'max'
        ]).reset_index()
        ...
        return {
            ...
            'title': f'Daily Load Profiles ({unit})',
            'data': chart_result.get('chart_data', {}),
            'average_pattern': {
                'hours': hourly_avg['Hour'].tolist(),
                'mean': hourly_avg['mean'].round(2).tolist(),
                'std': hourly_avg['std'].round(2).tolist(),
                'min': hourly_avg['min'].round(2).tolist(),
                'max': hourly_avg['max'].round(2).tolist()
            },
            'peak_hour': int(peak_hour),
            'min_hour': int(min_hour),
            'unit': unit,
            'metadata': {
                'analysis_type': 'daily_profiles',
                'sample_days': sample_days,
                'unit': unit
            }
        }
    ...
    def _calculate_enhanced_peak_statistics(self, df: pd.DataFrame, unit: str) -> Dict:
        """Calculate comprehensive peak statistics"""
        df_peaks = df.copy()
        
        # Use standardized column names: 'ds' for datetime and 'demand' for load values
        if 'ds' not in df_peaks.columns:
            raise ValueError("Missing 'ds' (datetime) column in DataFrame")
        if 'demand' not in df_peaks.columns:
            raise ValueError("Missing 'demand' column in DataFrame")
            
        # Ensure datetime column is properly formatted
        df_peaks['ds'] = pd.to_datetime(df_peaks['ds'])
        
        # Apply unit conversion
        unit_factor = self.analyzer.unit_factors.get(unit, 1)
        demand_values = df_peaks['demand'] * unit_factor
        
        # Top 10% peaks
        threshold = demand_values.quantile(0.9)
        peak_events = df_peaks[demand_values >= threshold].copy()
        peak_demand_values = demand_values[demand_values >= threshold]
        
        # Peak timing analysis
        peak_events['Hour'] = peak_events['ds'].dt.hour
        peak_events['DayOfWeek'] = peak_events['ds'].dt.day_name()
        peak_events['Month'] = peak_events['ds'].dt.month_name()
        
        peak_hours = peak_events['Hour'].value_counts().head(5)
        peak_days = peak_events['DayOfWeek'].value_counts().head(3)
        peak_months = peak_events['Month'].value_counts().head(3)
        
        return {
            'peak_threshold': float(threshold),
            'total_peak_events': len(peak_events),
            'average_peak': float(peak_demand_values.mean()),
            'highest_peak': float(peak_demand_values.max()),
            'peak_timing': {
                'common_hours': peak_hours.to_dict(),
                'common_days': peak_days.to_dict(),
                'common_months': peak_months.to_dict()
            },
            'unit': unit
        }

    def _calculate_load_factor_statistics(self, df: pd.DataFrame, period: str) -> Dict:
        """Calculate load factor statistics by period"""
        df_lf = df.copy()
        
        # Use standardized column names: 'ds' for datetime and 'demand' for load values
        if 'ds' not in df_lf.columns:
            raise ValueError("Missing 'ds' (datetime) column in DataFrame")
        if 'demand' not in df_lf.columns:
            raise ValueError("Missing 'demand' column in DataFrame")
            
        # Ensure datetime column is properly formatted
        df_lf['ds'] = pd.to_datetime(df_lf['ds'])
        
        if period == 'daily':
            df_lf['period'] = df_lf['ds'].dt.date
        elif period == 'weekly':
            df_lf['period'] = df_lf['ds'].dt.to_period('W')
        elif period == 'monthly':
            df_lf['period'] = df_lf['ds'].dt.to_period('M')
        else:
            df_lf['period'] = df_lf['ds'].dt.to_period('Q')
        
        # Calculate load factors
        period_stats = df_lf.groupby('period')['demand'].agg(['mean', 'max']).reset_index()
        period_stats['load_factor'] = (period_stats['mean'] / period_stats['max'] * 100)
        
        return {
            'average_load_factor': float(period_stats['load_factor'].mean()),
            'highest_load_factor': float(period_stats['load_factor'].max()),
            'lowest_load_factor': float(period_stats['load_factor'].min()),
            'load_factor_std': float(period_stats['load_factor'].std()),
            'periods_analyzed': len(period_stats),
            'period_type': period
        }


# Updated dashboard configuration for LoadProfileAnalysisService

def get_dashboard_data(self) -> Dict[str, Any]:
    """Enhanced dashboard data with comprehensive analysis types"""
    try:
        # Get available profiles with metadata
        profiles = self.get_available_profiles()
        ...
        # Calculate multi-year profiles
        multi_year_profiles = 0
        for profile in profiles:
            if profile.get('start_fy') and profile.get('end_fy'):
                if int(profile['end_fy']) - int(profile['start_fy']) > 1:
                    multi_year_profiles += 1
        ...
        return {
            ...
            'total_profiles': total_profiles,
            'total_size_mb': round(total_size_mb, 2),
            'multi_year_profiles': multi_year_profiles,
            'method_distribution': method_counts,
            'available_units': ['kW', 'MW', 'GW'],
            
            # Enhanced analysis types with detailed descriptions
            'analysis_types': [
                {
                    'id': 'overview', 
                    'name': 'Time Series Overview', 
                    'description': 'Complete load profile time series with peak/minimum identification and key statistics',
                    'chart_type': 'line',
                    'suitable_for': 'All profile types',
                    'data_requirements': 'Hourly data recommended'
                },
                {
                    'id': 'heatmap_hour_day', 
                    'name': 'Daily Pattern Heatmap', 
                    'description': 'Hour vs day-of-week heatmap showing weekly load patterns and identifying peak usage times',
                    'chart_type': 'heatmap',
                    'suitable_for': 'Multi-week data',
                    'data_requirements': 'At least 1 week of data'
                },
                {
                    'id': 'heatmap_month_year', 
                    'name': 'Seasonal Pattern Heatmap', 
                    'description': 'Month vs year heatmap revealing seasonal trends and year-over-year changes',
                    'chart_type': 'heatmap',
                    'suitable_for': 'Multi-year profiles',
                    'data_requirements': 'At least 1 year of data'
                },
                {
                    'id': 'heatmap_seasonal', 
                    'name': 'Seasonal Hourly Patterns', 
                    'description': 'Hour vs season heatmap showing how daily patterns change across seasons',
                    'chart_type': 'heatmap',
                    'suitable_for': 'Full year data',
                    'data_requirements': 'Complete seasonal data'
                },
                {
                    'id': 'peak_analysis', 
                    'name': 'Peak Demand Analysis', 
                    'description': 'Identification and analysis of top peak demand events with timing patterns',
                    'chart_type': 'bar',
                    'suitable_for': 'All profile types',
                    'data_requirements': 'Any duration'
                },
                {
                    'id': 'load_factor', 
                    'name': 'Load Factor Trends', 
                    'description': 'System efficiency analysis showing load factor evolution over time',
                    'chart_type': 'line_dual_axis',
                    'suitable_for': 'Long-term profiles',
                    'data_requirements': 'Multi-month data recommended'
                },
                {
                    'id': 'duration_curve', 
                    'name': 'Load Duration Curve', 
                    'description': 'Capacity utilization analysis showing demand distribution and percentiles',
                    'chart_type': 'line_markers',
                    'suitable_for': 'Capacity planning',
                    'data_requirements': 'Representative period'
                },
                {
                    'id': 'monthly', 
                    'name': 'Monthly Comparison', 
                    'description': 'Month-wise load comparison with averages, peaks, and load factors',
                    'chart_type': 'bar_line_combo',
                    'suitable_for': 'Multi-month data',
                    'data_requirements': 'At least 3 months'
                },
                {
                    'id': 'daily_profiles', 
                    'name': 'Daily Load Profiles', 
                    'description': 'Sample daily load curves showing typical and atypical daily patterns',
                    'chart_type': 'line_comparison',
                    'suitable_for': 'Pattern analysis',
                    'data_requirements': 'Multi-day data'
                }
            ],
            ...
            'comparison_types': [
                {
                    ...
                    'description': 'Basic load statistics comparison across profiles',
                    'metrics': ['peak', 'average', 'load_factor', 'total_energy']
                },
                {
                    'id': 'patterns', 
                    'name': 'Pattern Analysis', 
                    'description': 'Temporal pattern comparison including daily and seasonal patterns',
                    'metrics': ['peak_hours', 'seasonal_patterns', 'load_shapes']
                },
                {
                    'id': 'efficiency', 
                    'name': 'Efficiency Metrics', 
                    'description': 'Load factor and system efficiency comparison',
                    'metrics': ['load_factor', 'capacity_utilization', 'demand_variability']
                },
                {
                    'id': 'capacity', 
                    'name': 'Capacity Analysis', 
                    'description': 'Peak demand and capacity requirement comparison',
                    'metrics': ['peak_demand', 'coincident_peaks', 'diversity_factor']
                }
            ],
            
            # Filters with enhanced options
            'filter_options': {
                'units': ['kW', 'MW', 'GW'],
                'aggregations': ['hourly', 'daily', 'weekly', 'monthly'],
                'day_types': ['all', 'weekday', 'weekend', 'holiday'],
                'seasons': ['all', 'summer', 'monsoon', 'winter'],
            ...
            'quality_indicators': {
                'completeness_threshold': 95,  # Minimum % data completeness
                'outlier_threshold': 5,        # Maximum % outliers
                'zero_value_threshold': 10     # Maximum % zero values
            }
        }
        
    except Exception as e:
        logger.exception(f"Error getting enhanced dashboard data: {e}")
        return {'error': str(e)}

# Analysis type routing enhancement
def perform_analysis(self, profile_id: str, analysis_type: str, parameters: Dict[str, Any] = None) -> Dict[str, Any]:
    """Enhanced analysis with comprehensive routing"""
    try:
        ...
        if analysis_type == 'overview':
            result = self._perform_overview_analysis(df, unit, parameters)
        elif analysis_type in ['heatmap_hour_day', 'heatmap_month_year', 'heatmap_seasonal']:
            # Extract heatmap type from analysis_type
            heatmap_type = analysis_type.replace('heatmap_', '')
            heatmap_params = parameters.copy() if parameters else {}
            heatmap_params['heatmap_type'] = heatmap_type
            result = self._perform_heatmap_analysis(df, unit, heatmap_params)
        elif analysis_type == 'peak_analysis':
            result = self._perform_peak_analysis(df, unit, parameters)
        elif analysis_type == 'load_factor':
            result = self._perform_load_factor_analysis(df, unit, parameters)
            ...
            result = self._perform_monthly_analysis(df, unit, parameters)
        elif analysis_type == 'daily_profiles':
            result = self._perform_daily_profiles_analysis(df, unit, parameters)
        else:
        ...
        result['metadata'] = {
            ...
            'parameters': parameters,
            'generated_at': datetime.now().isoformat(),
            'fiscal_years': sorted(df['Fiscal_Year'].unique().tolist()) if 'Fiscal_Year' in df.columns else [],
            'date_range': {
                'start': df['ds'].min().isoformat() if 'ds' in df.columns else None,
                'end': df['ds'].max().isoformat() if 'ds' in df.columns else None
            } if 'ds' in df.columns else None
...
# Enhanced validation for multi-year profiles
def validate_analysis_suitability(self, profile_id: str, analysis_type: str) -> Dict[str, Any]:
    """Validate if analysis type is suitable for the profile data"""
    try:
        df = self.analyzer.load_profile_data(profile_id)
        ...
        data_span_days = (df['ds'].max() - df['ds'].min()).days if 'ds' in df.columns else 0
        unique_fiscal_years = df['Fiscal_Year'].nunique() if 'Fiscal_Year' in df.columns else 1
        total_records = len(df)
        
        suitability = {'suitable': True, 'warnings': [], 'recommendations': []}
        
        # Analysis-specific validations
        if analysis_type in ['heatmap_month_year'] and unique_fiscal_years < 2:
            suitability['warnings'].append('Multi-year heatmap works best with multiple fiscal years')
        
        if analysis_type == 'heatmap_hour_day' and data_span_days < 7:
            suitability['suitable'] = False
            suitability['reason'] = 'At least 1 week of data required for daily pattern heatmap'
        
        if analysis_type == 'monthly' and data_span_days < 90:
            suitability['warnings'].append('Monthly analysis works best with at least 3 months of data')
        
        if analysis_type == 'load_factor' and data_span_days < 30:
            suitability['warnings'].append('Load factor trends more meaningful with longer time periods')
        
        if total_records < 24:
            suitability['suitable'] = False
            suitability['reason'] = 'Insufficient data points for meaningful analysis'
        
        # Add recommendations based on data characteristics
        if unique_fiscal_years > 1:
            suitability['recommendations'].append('Consider year-over-year comparison analysis')
        
        if data_span_days > 365:
            suitability['recommendations'].append('Seasonal analysis highly recommended for long-term data')
        
        return suitability
        
    except Exception as e:
        logger.error(f"Error validating analysis suitability: {e}")
        return {'suitable': False, 'reason': f'Validation error: {str(e)}'}